...
const Home = () => {
  ...
  useEffect(() => {
    const scrollContainers = [trendingRef, popularRef, topRatedRef, upcomingRef];
      ...
      <div className="mb-12">
        ...
        {loading ? (
          <div className="flex space-x-4 overflow-x-auto pb-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="flex-shrink-0 w-[240px] h-[360px] bg-gray-800 rounded animate-pulse"></div>
            ))}
          </div>
        ) : (
                  ...
                  <div className="overflow-hidden rounded shadow-md cursor-pointer transform transition group-hover/item:scale-105 group-hover/item:shadow-xl">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover/item:opacity-100 transition-opacity flex flex-col justify-end p-4">
                      ...
                      <div className="flex items-center space-x-2 mt-2">
                        <span className="text-green-500 text-sm">{Math.round(item.vote_average * 10)}% Match</span>
                        <span className="text-xs border px-1">{item.adult ? "18+" : "PG"}</span>
                        <span className="text-xs">{item.release_date?.substring(0, 4) || item.first_air_date?.substring(0, 4)}</span>
                      </div>
        ...
        <div className="flex items-center">
          ...
          <nav className="ml-8 hidden md:flex">
            <ul className="flex space-x-6">
              ...
              <li className="text-sm text-gray-300 hover:text-white">TV Shows</li>
              <li className="text-sm text-gray-300 hover:text-white">Movies</li>
              <li className="text-sm text-gray-300 hover:text-white">New & Popular</li>
              <li className="text-sm text-gray-300 hover:text-white">My List</li>
            </ul>
          </nav>
        </div>
      ...
      {trendingContent.length > 0 && (
        <div className="relative h-[80vh] w-full">
          ...
          <div className="relative h-full flex flex-col justify-end pb-20 px-12">
            <h1 className="text-5xl font-bold max-w-2xl mb-4">{trendingContent[0]?.title || trendingContent[0]?.name}</h1>
            <p className="text-lg max-w-xl mb-6">{trendingContent[0]?.overview?.substring(0, 150)}...</p>
            <div className="flex space-x-4">
              ...
              <button className="bg-gray-600/70 text-white px-6 py-2 rounded flex items-center font-medium hover:bg-gray-500/70 transition">
                <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                  <path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4.707 3.293a1 1 0 010 1.414l-1 1a1 1 0 11-1.414-1.414l1-1a1 1 0 011.414 0zm-9.193 9.193a1 1 0 010 1.414l-1 1a1 1 0 11-1.414-1.414l1-1a1 1 0 011.414 0zM5 10a1 1 0 01-1 1H3a1 1 0 110-2h1a1 1 0 011 1zm14 0a1 1 0 01-1 1h-1a1 1 0 110-2h1a1 1 0 011 1zm-7 4a1 1 0 01-1 1h-2a1 1 0 110-2h2a1 1 0 011 1zm-2-8a1 1 0 011-1h2a1 1 0 110 2h-2a1 1 0 01-1-1z" clipRule="evenodd"></path>
                </svg>
                More Info
              </button>
            </div>
